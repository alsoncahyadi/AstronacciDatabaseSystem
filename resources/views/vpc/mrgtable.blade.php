							...
							@foreach ($clients as $client)
								<tr>
									@foreach ($attsMaster as $attMaster)
										@if ($attMaster == 'name')
 										<td class="fixed-side" style="white-space: nowrap; min-width: 160px; pointer-events: auto !important;"> <a id="{{$attMaster}}_{{$client->master_id}}" target="_blank" href="{{route('detail', ['id' => $client->master_id])}}" style="text-decoration:none; color:black;"> {{ $client->$attMaster }}</a></td>
									...
									@endforeach
									@foreach ($atts as $att)
										<td style="max-width: 100px; white-space: nowrap;cursor:context-menu;">{{$client->$att}}</td>
									@endforeach
								</tr>
							<?php $idx = $idx + 1; ?>
							@endforeach
